...
export default {
  ...
  hydrogenResponseToCommentResults(hydrogenResponse, curr, last) {
    const e = atom.workspace.getActiveTextEditor();
    // for DEBUG
    console.log(
  ...
  promiseToCommentResults(hydrogenPromise, curr, last) {
    const e = atom.workspace.getActiveTextEditor();

    // Get a promise from Hydrogen. If none available, bail.
    ...
    return hydrogenPromise.then(
        hydrogenResponse => this.hydrogenResponseToCommentResults(
            hydrogenResponse, curr, last));
  },
  ...
  hydrogenThisCodeblock() {
    ...
    // Then invoke Hydrogen: Run on it.
    this.dispatchEditorCommand('hydrogen:run');

    return this.promiseToCommentResults(
        this.hydrogenCallbackWrapper(), curr, last);
  },
    ...
    function whee(start) {
      e.scanInBufferRange(
          /(~~~|```)js.*?\n([\s\S]*?)\1$/, [start, end()], obj => {
            ...
            console.log('MOVING TO ', newStart, 'from START', start);

            return that.hydrogenThisCodeblock().then(_ => whee(newStart));
          });
    }
    whee([0, 0]);
